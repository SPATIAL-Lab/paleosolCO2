...
blue_co2 = blue_co2 |>
  ...
  mutate(age = age / 1e3)
D47 = read_csv("data/CLP_data/fuxian_D47.csv")
D47$position = 5
...
post_ts_co2 = data.frame(ages, t(apply(post.ts$BUGSoutput$sims.list$pCO2, 2, quantile,
                                   c(.16, .5, .84))))
names(post_ts_co2) = c("age", "lower", "median", "higher")
post_ts_MAT = data.frame(ages, t(apply(post.ts$BUGSoutput$sims.list$MAT, 2, quantile,
                                   c(.16, .5, .84))))
names(post_ts_MAT) = c("age", "lower", "median", "higher")
# JPI-CS
load("out/ts_fuxian_D47_MS_ECS_1e4.rda")
post_ts_ecs_co2 = data.frame(ages, t(apply(post.ts$BUGSoutput$sims.list$pCO2, 2, quantile,
                                   c(.16, .5, .84))))
names(post_ts_ecs_co2) = c("age", "lower", "median", "higher")
post_ts_ecs_MAT = data.frame(ages, t(apply(post.ts$BUGSoutput$sims.list$MAT, 2, quantile,
                                                       c(.16, .5, .84))))
names(post_ts_ecs_MAT) = c("age", "lower", "median", "higher")

post_ts_co2 = post_ts_co2 |>
  filter(age >= -2.6 & age < 0) |>
  mutate(age = -age)
post_ts_ecs_co2 = post_ts_ecs_co2 |>
  filter(age >= -2.6 & age < 0) |>
  mutate(age = -age)
...
post_ts_MAT = post_ts_MAT |>
  filter(age >= -2.6 & age < 0) |>
  mutate(age = -age)
post_ts_ecs_MAT = post_ts_ecs_MAT |>
  filter(age >= -2.6 & age < 0) |>
  mutate(age = -age)

p4 = ggplot(post_ts_MAT, aes(x = age, y = median)) +
  geom_ribbon(aes(ymin = lower,
                  ymax = higher),
              fill = "tomato", alpha = .2) +
  geom_ribbon(data = post_ts_ecs_MAT,
              aes(ymin = lower,
                  ymax = higher),
              fill = "royalblue", alpha = .2) +
  geom_line(data = gdgt, aes(x = age, y = MAST),
  ...
  geom_line(data = post_ts_ecs_MAT,
            aes(x = age, y = median),
