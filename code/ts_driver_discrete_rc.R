...
md = read.csv("data/d13Ca_tipple.csv") %>% filter(age <= 7.8 & age >= 2.8)
clp$d13C.stdev = 0.2
clp$d18O.stdev = 0.2
clp$d13Co.stdev = 0.2
md$d13Ca.stdev = 0.05
...
system.time({post.clp = jags.parallel(d, NULL, parms, "code/models/time_series_discrete_rc.R", 
                                      n.iter = 1e5, n.chains = 3, n.burnin = 3e4)})

sum_clp = post.clp$BUGSoutput$summary
# save(post.clp, file = "out/ts_jx_1e5.rda")
# load("out/ts_fx_5e5.rda")
