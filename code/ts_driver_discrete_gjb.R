...
parms = c("pCO2", "MAT", "Tsoil", "tsc", "MAP", "PCQ_pf", "d18.p", "ETR", "D13Cr",
          "d13Ca", "PCQ_to", "AET_PCQ", "f_R", "S_z", "R_PCQ_S_0", "z.ef",
          "L", "pore", "AI")

system.time({post.ts = jags.parallel(d, NULL, parms, "code/models/time_series_discrete_v2_gjb.R", 
                        n.iter = 1e5, n.chains = 3, n.burnin = 0.5e5)})

View(post.ts$BUGSoutput$summary)

sl = post.ts$BUGSoutput$sims.list[names(sl) != "deviance"]
for(i in seq_along(sl)){
  plot.jpi(ages, sl[[i]], ylab = names(sl)[i])
}

save(post.ts, file = "out/ts.rda")
