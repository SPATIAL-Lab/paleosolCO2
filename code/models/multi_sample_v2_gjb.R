model{
  ...
  for(i in 1:length(ai)){  
    ...
    # R_PCQ_D_m1[i] = 1.25 * exp(0.07987 * Tair_PCQ[i]) * PPCQ[i] / (29.86 + PPCQ[i]) # CLP model
    ...
    R18p[i] = (d18.p[i] / 1000 + 1) * R18.VSMOW
    ...
    d18O.s[i] = ((R18s[i] / R18.VSMOW) - 1) * 1000
  ...
  for(i in 1:length(ai)){
    ...
    PCQ_to[i] ~ dunif(7, 15)
    MAP[i] ~ dunif(200, 1200) # mean annual precipitation, mm
    PCQ_pf[i] ~ dunif(0.02, 0.5) # PCQ precipitation fraction
#    Tair_OOS[i] = (4 * MAT[i] - Tair_PCQ[i]) / 3
#    d18p_PCQ[i] = -15 + 0.58 * Tair_PCQ[i]
#    d18p_OOS[i] = -15 + 0.58 * Tair_OOS[i]
#    d18p_m[i] = (d18p_PCQ[i] * PCQ_pf[i] + d18p_OOS[i] * (1 - spre[i]) * (1 - PCQ_pf[i])) / (PCQ_pf[i] + (1 - spre[i]) * (1 - PCQ_pf[i]))
#    d18p[i] ~ dnorm(d18p_m[i], 1 / 3 ^ 2)
    ...
    f_R[i] ~ dbeta(10, 50) # ratio of PCQ to mean annual respiration rate
    ETR[i] ~ dbeta(0.06 * 1000 / 0.94, 1e3) # Soil evaporation / AET
#    spre[i] ~ dbeta(0.55 * 500 / 0.45, 500)
