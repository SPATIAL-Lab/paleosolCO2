model{
  ...
  for(i in 2:length(ai)){
    ...
    # pCO2[i] = max(min(pCO2.p[i], 500), 150)
    ...
    # PCQ_to[i] = max(min(PCQ_to.p[i], 16), 10)
  ...
  pCO2.tau ~ dgamma(10, 1e4) # 10, 10e3
  pCO2.phi ~ dbeta(2, 5)
  ...
  PCQ_pf.tau ~ dgamma(10, 1e-2) # percentage
  PCQ_pf.phi ~ dbeta(2, 5)

  tsc.tau ~ dgamma(10, 1e-6) ## was 1e-3
  tsc.phi ~ dbeta(2, 5)

  # ha.tau ~ dgamma(10, 1e-2)
  # ha.phi ~ dbeta(2, 5)

  f_R.tau ~ dgamma(10, 1e-7) # percentage
  f_R.phi ~ dbeta(2, 5)
  ...
  PCQ_to.eps[1] = 0
  MAP[1] ~ dunif(200, 1200) # mean annual precipitation, mm
  MAP.eps[1] = 0
  PCQ_pf[1] ~ dunif(0.02, 0.5) # PCQ precipitation fraction
  PCQ_pf.eps[1] = 0

  ## Secondary soil
  tsc[1] ~ dunif(0.34, 0.55) # seasonal offset of PCQ for thermal diffusion
  tsc.eps[1] = 0
  f_R[1] ~ dbeta(10, 50) # ratio of PCQ to mean annual respiration rate
  f_R.eps[1] = 0
